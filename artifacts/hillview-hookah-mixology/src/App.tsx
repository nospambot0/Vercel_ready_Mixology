  ...
  return (
    <main className="hv-shell hv-page-in pb-28"><div className="mx-auto max-w-5xl">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.1fr]"><section className="hv-surface rounded-[2rem] p-5 text-center md:p-8"><SectionEyebrow>SCAN TO PAY</SectionEyebrow><div className="mx-auto flex max-w-sm flex-col items-center"><div className="rounded-[1.75rem] bg-white p-4 shadow-xl"><img src={qrUrl} alt={`UPI payment QR for ₹${bill.amount.toFixed(2)}`} className="h-72 w-72 object-contain" /></div><p className="mt-5 text-4xl font-black">₹{bill.amount.toFixed(2)}</p><p className="mt-2 text-xs text-muted-foreground">{bill.remark || 'Mixology POS payment'}</p><p className="mt-4 rounded-full bg-muted px-3 py-1 font-mono text-[10px]">{bill.id}</p></div></section>
        ...
        {bill.status === 'PAID' && <section className="hv-surface rounded-[2rem] p-6 print:block lg:col-span-2" data-testid="pos-acknowledgement"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><SectionEyebrow>ACKNOWLEDGEMENT</SectionEyebrow><h2 className="hv-display text-4xl">Payment Received</h2><p className="mt-2 text-sm text-muted-foreground">Thank you for your payment at Mixology.</p></div><div className="rounded-2xl bg-secondary/10 px-4 py-3 text-center"><p className="text-[10px] font-bold uppercase text-muted-foreground">Status</p><p className="font-black text-secondary">PAID</p></div></div><div className="mt-6 grid gap-3 border-t border-border pt-5 text-sm sm:grid-cols-2"><p><span className="text-muted-foreground">Bill:</span> {bill.id}</p><p><span className="text-muted-foreground">Amount:</span> ₹{bill.amount.toFixed(2)}</p><p><span className="text-muted-foreground">Payment:</span> UPI</p><p><span className="text-muted-foreground">Paid to:</span> {PAYEE_UPI}</p><p><span className="text-muted-foreground">Date:</span> {new Date(bill.createdAt).toLocaleString('en-IN')}</p><p><span className="text-muted-foreground">Remark:</span> {bill.remark || '—'}</p></div></section>}
      </div>}
    </div></main>
  );
}
