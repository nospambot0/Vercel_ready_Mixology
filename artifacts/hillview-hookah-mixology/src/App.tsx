...
function PosPage() {
  ...
  const createBill = () => {
    ...
    const id = `HV-${Date.now().toString(36).toUpperCase()}`;
    setBill({ id, amount: Math.round(value * 100) / 100, remark: remark.trim(), createdAt: new Date().toISOString(), status: 'PENDING' });
  };
  const resetBill = () => { setBill(null); setAmount(''); setRemark(''); };
  const upiUrl = bill ? `upi://pay?pa=${encodeURIComponent(PAYEE_UPI)}&pn=${encodeURIComponent('Hillview Hookah Mixology')}&am=${bill.amount.toFixed(2)}&cu=INR&tr=${encodeURIComponent(bill.id)}&tn=${encodeURIComponent(bill.remark || 'Hillview POS payment')}` : '';
  ...
  return (
    <main className="hv-shell hv-page-in pb-28"><div className="mx-auto max-w-5xl">
      <div className="mb-6 flex items-center justify-between gap-3"><div><SectionEyebrow>POINT OF SALE</SectionEyebrow><h1 className="hv-display text-5xl md:text-7xl">Hillview POS</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Create a bill, show a dynamic UPI QR with the exact amount, then manually confirm payment.</p></div><Link href="/manage" className="hidden min-h-10 items-center gap-2 rounded-xl border border-border px-4 text-xs font-bold hover:bg-muted md:inline-flex"><ArrowLeft size={15} /> Dashboard</Link></div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.1fr]"><section className="hv-surface rounded-[2rem] p-5 text-center md:p-8"><SectionEyebrow>SCAN TO PAY</SectionEyebrow><div className="mx-auto flex max-w-sm flex-col items-center"><div className="rounded-[1.75rem] bg-white p-4 shadow-xl"><img src={qrUrl} alt={`UPI payment QR for ₹${bill.amount.toFixed(2)}`} className="h-72 w-72 object-contain" /></div><p className="mt-5 text-4xl font-black">₹{bill.amount.toFixed(2)}</p><p className="mt-2 text-xs text-muted-foreground">{bill.remark || 'Hillview POS payment'}</p><p className="mt-4 rounded-full bg-muted px-3 py-1 font-mono text-[10px]">{bill.id}</p></div></section>
        <section className="hv-surface rounded-[2rem] p-5 md:p-8"><div className={`rounded-2xl border p-5 ${bill.status === 'PAID' ? 'border-secondary/40 bg-secondary/10' : 'border-border bg-muted/30'}`}><div className="flex items-center gap-3">{bill.status === 'PAID' ? <CheckCircle2 className="text-secondary" size={24} /> : <QrCode className="text-secondary" size={24} />}<div><p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{bill.status === 'PAID' ? 'Payment confirmed' : 'Awaiting payment'}</p><p className="font-semibold">{bill.status === 'PAID' ? 'Bill marked PAID' : 'Customer can scan the QR now'}</p></div></div></div>
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-2"><button onClick={() => window.print()} className="min-h-12 rounded-2xl bg-primary px-5 text-sm font-bold text-primary-foreground"><Printer size={16} className="mr-2 inline" /> Print Acknowledgement</button><button onClick={resetBill} className="min-h-12 rounded-2xl border border-border px-5 text-sm font-bold hover:bg-muted"><RefreshCw size={16} className="mr-2 inline" /> New Bill</button></div>}
        </section>
        {bill.status === 'PAID' && <section className="hv-surface rounded-[2rem] p-6 print:block lg:col-span-2" data-testid="pos-acknowledgement"><div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"><div><SectionEyebrow>ACKNOWLEDGEMENT</SectionEyebrow><h2 className="hv-display text-4xl">Payment Received</h2><p className="mt-2 text-sm text-muted-foreground">Thank you for your payment at Hillview Hookah Mixology.</p></div><div className="rounded-2xl bg-secondary/10 px-4 py-3 text-center"><p className="text-[10px] font-bold uppercase text-muted-foreground">Status</p><p className="font-black text-secondary">PAID</p></div></div><div className="mt-6 grid gap-3 border-t border-border pt-5 text-sm sm:grid-cols-2"><p><span className="text-muted-foreground">Bill:</span> {bill.id}</p><p><span className="text-muted-foreground">Amount:</span> ₹{bill.amount.toFixed(2)}</p><p><span className="text-muted-foreground">Payment:</span> UPI</p><p><span className="text-muted-foreground">Paid to:</span> {PAYEE_UPI}</p><p><span className="text-muted-foreground">Date:</span> {new Date(bill.createdAt).toLocaleString('en-IN')}</p><p><span className="text-muted-foreground">Remark:</span> {bill.remark || '—'}</p></div></section>}
      </div>}
    </div></main>
  );
}
